...
-- Githubからファイルを持ってくる
COPY FILES INTO @SNOWRETAIL_DB.SNOWRETAIL_SCHEMA.FILE FROM @GIT_INTEGRATION_FOR_HANDSON/branches/pri_20250509/data/;
COPY FILES INTO @SNOWRETAIL_DB.SNOWRETAIL_SCHEMA.SEMANTIC_MODEL_STAGE FROM @GIT_INTEGRATION_FOR_HANDSON/branches/pri_20250509/handson2/sales_analysis_model.yaml;

// Step4: NotebookとStreamlitを作成 //

-- Notebookの作成
CREATE OR REPLACE NOTEBOOK cortex_handson_part1
    FROM @GIT_INTEGRATION_FOR_HANDSON/branches/pri_20250509/handson1
...
CREATE OR REPLACE STREAMLIT sis_snowretail_voc_analysis_dev
    FROM @GIT_INTEGRATION_FOR_HANDSON/branches/pri_20250509/handson2
